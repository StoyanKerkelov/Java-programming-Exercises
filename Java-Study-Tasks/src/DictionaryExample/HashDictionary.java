...
/**
 * Implementation of {@link Dictionary} interface using hash table. Collisions
...
public class HashDictionary<K, V> implements Dictionary<K, V> {
	...
	public V put(K key, V value) {
		...
		for (int i = 0; i < chain.size(); i++) {
			...
			if (entry.getKey().equals(key)) {
				// Key found -> replace its value with the new
				// value
				DictionaryEntry<K, V> newEntry = new DictionaryEntry<K, V>(
						key, value);
		...
		for (List<DictionaryEntry<K, V>> oldChain : oldTable) {
			if (oldChain != null) {
				for (DictionaryEntry<K, V> dictionaryEntry : oldChain) {
					List<DictionaryEntry<K, V>> chain = findChain(
	...
	public Iterator<DictionaryEntry<K, V>> iterator() {
		List<DictionaryEntry<K, V>> entries = new ArrayList<DictionaryEntry<K, V>>(
				this.table.length);
